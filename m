Return-Path: <iommu-bounces@lists.linux-foundation.org>
X-Original-To: lists.iommu@lfdr.de
Delivered-To: lists.iommu@lfdr.de
Received: from mail.linuxfoundation.org (mail.linuxfoundation.org [140.211.169.12])
	by mail.lfdr.de (Postfix) with ESMTPS id DE4269C9F5
	for <lists.iommu@lfdr.de>; Mon, 26 Aug 2019 09:14:26 +0200 (CEST)
Received: from mail.linux-foundation.org (localhost [127.0.0.1])
	by mail.linuxfoundation.org (Postfix) with ESMTP id BB4D8102A;
	Mon, 26 Aug 2019 07:14:24 +0000 (UTC)
X-Original-To: iommu@lists.linux-foundation.org
Delivered-To: iommu@mail.linuxfoundation.org
Received: from smtp1.linuxfoundation.org (smtp1.linux-foundation.org
	[172.17.192.35])
	by mail.linuxfoundation.org (Postfix) with ESMTPS id 4D2851020
	for <iommu@lists.linux-foundation.org>;
	Mon, 26 Aug 2019 07:14:11 +0000 (UTC)
X-Greylist: whitelisted by SQLgrey-1.7.6
Received: from mail-wr1-f67.google.com (mail-wr1-f67.google.com
	[209.85.221.67])
	by smtp1.linuxfoundation.org (Postfix) with ESMTPS id EB8F689B
	for <iommu@lists.linux-foundation.org>;
	Mon, 26 Aug 2019 07:14:09 +0000 (UTC)
Received: by mail-wr1-f67.google.com with SMTP id q12so14170334wrj.12
	for <iommu@lists.linux-foundation.org>;
	Mon, 26 Aug 2019 00:14:09 -0700 (PDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
	d=monstr-eu.20150623.gappssmtp.com; s=20150623;
	h=reply-to:subject:to:references:from:openpgp:autocrypt:message-id
	:date:user-agent:mime-version:in-reply-to;
	bh=o68j6SzsLNai8uiHGIYdY0sQQpDhejxLSmbrKqJ8oCc=;
	b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
	d=1e100.net; s=20161025;
	h=x-gm-message-state:reply-to:subject:to:references:from:openpgp
	:autocrypt:message-id:date:user-agent:mime-version:in-reply-to;
	bh=o68j6SzsLNai8uiHGIYdY0sQQpDhejxLSmbrKqJ8oCc=;
	b=
X-Gm-Message-State: APjAAAVgR+ev2Emff4ldlndi0hLqRBEKJsQPe4iBMCCLx3BEW0p6XTXy
	muc8cSCZpfKcKHg4lXrhypUDVg==
X-Google-Smtp-Source: APXvYqztR4ESgII7oaRjQBsF8++xcMJsaHGGSj0fgdMl45+LFwijN6+bdegi2usSIwGKZtThScKEXA==
X-Received: by 2002:adf:f386:: with SMTP id m6mr8622514wro.313.1566803648204; 
	Mon, 26 Aug 2019 00:14:08 -0700 (PDT)
Received: from [64.233.166.108] ([149.199.62.131])
	by smtp.gmail.com with ESMTPSA id
	a26sm10767495wmg.45.2019.08.26.00.14.05
	(version=TLS1_2 cipher=ECDHE-RSA-AES128-GCM-SHA256 bits=128/128);
	Mon, 26 Aug 2019 00:14:07 -0700 (PDT)
Subject: Re: convert microblaze to the generic dma remap allocator
To: Christoph Hellwig <hch@infradead.org>, iommu@lists.linux-foundation.org,
	linux-kernel@vger.kernel.org
References: <20190814140348.3339-1-hch@lst.de>
	<20190826070258.GA31420@infradead.org>
From: Michal Simek <monstr@monstr.eu>
Openpgp: preference=signencrypt
Autocrypt: addr=monstr@monstr.eu; prefer-encrypt=mutual; keydata=
	
	Qg41tpHgK85MlNXw6MBWLgdXBSGdD0zVX4S4Gz+vwyY1
Message-ID: <8da3e42e-6c13-104f-b985-d04be1eca018@monstr.eu>
Date: Mon, 26 Aug 2019 09:13:54 +0200
User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:60.0) Gecko/20100101
	Thunderbird/60.8.0
MIME-Version: 1.0
In-Reply-To: <20190826070258.GA31420@infradead.org>
X-Spam-Status: No, score=-1.9 required=5.0 tests=BAYES_00,DKIM_SIGNED,
	DKIM_VALID,RCVD_IN_DNSWL_NONE autolearn=ham version=3.3.1
X-Spam-Checker-Version: SpamAssassin 3.3.1 (2010-03-16) on
	smtp1.linux-foundation.org
X-BeenThere: iommu@lists.linux-foundation.org
X-Mailman-Version: 2.1.12
Precedence: list
List-Id: Development issues for Linux IOMMU support
	<iommu.lists.linux-foundation.org>
List-Unsubscribe: <https://lists.linuxfoundation.org/mailman/options/iommu>,
	<mailto:iommu-request@lists.linux-foundation.org?subject=unsubscribe>
List-Archive: <http://lists.linuxfoundation.org/pipermail/iommu/>
List-Post: <mailto:iommu@lists.linux-foundation.org>
List-Help: <mailto:iommu-request@lists.linux-foundation.org?subject=help>
List-Subscribe: <https://lists.linuxfoundation.org/mailman/listinfo/iommu>,
	<mailto:iommu-request@lists.linux-foundation.org?subject=subscribe>
Reply-To: monstr@monstr.eu
Content-Type: multipart/mixed; boundary="===============6595145831559917247=="
Sender: iommu-bounces@lists.linux-foundation.org
Errors-To: iommu-bounces@lists.linux-foundation.org

This is an OpenPGP/MIME signed message (RFC 4880 and 3156)
--===============6595145831559917247==
Content-Type: multipart/signed; micalg=pgp-sha1;
 protocol="application/pgp-signature";
 boundary="AauZJ7EApEWIQo0pU1Lj4WF5ziYtX45ik"

This is an OpenPGP/MIME signed message (RFC 4880 and 3156)
--AauZJ7EApEWIQo0pU1Lj4WF5ziYtX45ik
Content-Type: multipart/mixed; boundary="YSQd0AYkgxTbVVAQpSYOw0eAKZr8Stao9";
 protected-headers="v1"
From: Michal Simek <monstr@monstr.eu>
Reply-To: monstr@monstr.eu
To: Christoph Hellwig <hch@infradead.org>, iommu@lists.linux-foundation.org,
 linux-kernel@vger.kernel.org
Message-ID: <8da3e42e-6c13-104f-b985-d04be1eca018@monstr.eu>
Subject: Re: convert microblaze to the generic dma remap allocator
References: <20190814140348.3339-1-hch@lst.de>
 <20190826070258.GA31420@infradead.org>
In-Reply-To: <20190826070258.GA31420@infradead.org>

--YSQd0AYkgxTbVVAQpSYOw0eAKZr8Stao9
Content-Type: text/plain; charset=utf-8
Content-Language: en-US
Content-Transfer-Encoding: quoted-printable

Hi,

On 26. 08. 19 9:02, Christoph Hellwig wrote:
> On Wed, Aug 14, 2019 at 04:03:46PM +0200, Christoph Hellwig wrote:
>> Hi Michal,
>>
>> can you take a look at this patch that moves microblaze over to the
>> generic DMA remap allocator?  I've been trying to slowly get all
>> architectures over to the generic code, and microblaze is one that
>> seems very straightfoward to convert.
>=20
> Michal, any chance you could look over this series?
>=20

let me take a look.

M


--=20
Michal Simek, Ing. (M.Eng), OpenPGP -> KeyID: FE3D1F91
w: www.monstr.eu p: +42-0-721842854
Maintainer of Linux kernel - Xilinx Microblaze
Maintainer of Linux kernel - Xilinx Zynq ARM and ZynqMP ARM64 SoCs
U-Boot custodian - Xilinx Microblaze/Zynq/ZynqMP/Versal SoCs



--YSQd0AYkgxTbVVAQpSYOw0eAKZr8Stao9--

--AauZJ7EApEWIQo0pU1Lj4WF5ziYtX45ik
Content-Type: application/pgp-signature; name="signature.asc"
Content-Description: OpenPGP digital signature
Content-Disposition: attachment; filename="signature.asc"

-----BEGIN PGP SIGNATURE-----

iF0EARECAB0WIQQbPNTMvXmYlBPRwx7KSWXLKUoMIQUCXWOGtgAKCRDKSWXLKUoM
IThBAKCBYM2K/oYTt4T5kmc0XAG2siOuJACeKxzMFnOy2OXyNqMCQCXR7J47mAU=
=6Rxt
-----END PGP SIGNATURE-----

--AauZJ7EApEWIQo0pU1Lj4WF5ziYtX45ik--

--===============6595145831559917247==
Content-Type: text/plain; charset="us-ascii"
MIME-Version: 1.0
Content-Transfer-Encoding: 7bit
Content-Disposition: inline

_______________________________________________
iommu mailing list
iommu@lists.linux-foundation.org
https://lists.linuxfoundation.org/mailman/listinfo/iommu
--===============6595145831559917247==--
